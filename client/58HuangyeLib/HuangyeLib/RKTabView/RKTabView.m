...
#define TAB_IMAGE_BADGE     [UIImage imageNamed:@"ProgressHUD.bundle/badge.png"]
...
- (UIControl *)existingTabForTabItem:(RKTabItem *)tabItem {
    ...
    if (index != NSNotFound && self.tabViews.count > index) {
        return [self.tabViews objectAtIndex: [self indexOfTab:tabItem]];
    } else {
        return nil;
    }
}
...
- (RKTabItem *)tabItemForTab:(UIControl *)tab {
    return [self.tabItems objectAtIndex:[self.tabViews indexOfObject:tab]];
}
...
//        if ( [titleLabel respondsToSelector:@selector(setAdjustsLetterSpacingToFitWidth:) ] ) {
...
//        }
        ...
        if (tabItem.currentStateFontColor) {
            textColor = tabItem.currentStateFontColor;
        } else if (!tabItem.currentStateFontColor && self.titlesFontColor) {
            textColor = self.titlesFontColor;
        }
        titleLabel.textColor = textColor;
        
        titleSize = [tabItem.titleString sizeWithFont:titleLabel.font constrainedToSize:CGSizeMake(tab.bounds.size.width, MAXFLOAT) lineBreakMode:UILineBreakModeWordWrap];
    ...
    if ( tabItem.badgeValue > 0 ) {
        UIImageView *badgeView = [[UIImageView alloc ] initWithImage:TAB_IMAGE_BADGE ] ;
        badgeView.center = CGPointMake( tabItem.imageForCurrentState.size.width - 2 , 2 ) ;
        [((UIView *)interfaceElement) addSubview:badgeView ] ;
    }
